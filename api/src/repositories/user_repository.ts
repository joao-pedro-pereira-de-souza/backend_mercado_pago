...
interface ParamsCreateInterface
  extends Pick<User, 'name' | 'email' | 'password' | 'id_permission'> {
  photo?: string | null | undefined;
}


interface ParamsCreateClientInterface
  extends Pick<User, 'name' | 'email' | 'password' | 'id_permission'> {
  photo?: string | null | undefined;
  cpf: string;
}
...
class UserRepository {
    ...
    async createUserClient(data: ParamsCreateClientInterface) {
        const { cpf, ...dataUser } = data;
